...
from extractor.agents.pk_pe_agents.pk_pe_identification_step import PKPEIdentificationStep, PKPEIdentificationState


def test_pk_pe_identification_step_22050870(llm, step_callback):
    step = PKPEIdentificationStep()
    state = PKPEIdentificationState(
        llm=llm,
        step_output_callback=step_callback,
        title="Pharmacokinetics of intravenous lorazepam in pediatric patients with and without status epilepticus",
        abstract="""Objective: To evaluate the single dose pharmacokinetics of an intravenous dose of lorazepam in pediatric patients treated for status epilepticus (SE) or with a history of SE.
