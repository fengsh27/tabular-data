...
class ArticleStamper(Stamper):
    ...
    def _ensure_pmid_folder_exist(self):
        ...
        try:
            ...
            logger.error(e)
            logger.error(
                f"pmid: {self._pmid}, name: {self.name}, pmid_folder: {self.pmid_folder}, output_folder: {self.output_folder}"
            )
            if self.name is None:
                self.name = "unknown"
    ...
    @staticmethod
    def _now_string(
        include_ms: Optional[bool] = False, in_filename: Optional[bool] = False
    ):
        now = datetime.now()
        if not in_filename:
            return (
