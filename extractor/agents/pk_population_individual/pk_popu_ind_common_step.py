...
from extractor.agents.pk_population_individual.pk_popu_ind_workflow_utils import (
    pk_popu_ind_enter_step,
    pk_popu_ind_leave_step,
)
from extractor.agents.pk_population_individual.pk_popu_ind_common_agent import (
    PKPopuIndCommonAgentResult,
    PKPopuIndCommonAgent,
)
...
class PKPopuIndCommonStep(ABC):
    ...
    def enter_step(self, state: PKPopuIndWorkflowState):
        pk_popu_ind_enter_step(state, self.start_title, self.start_descp)

    def leave_step(
        self,
        state: PKPopuIndWorkflowState,
        res: PKPopuIndCommonAgentResult | None = None,
    ...
    def _get_previous_errors_prompt(self, state: PKPopuIndWorkflowState) -> str:
    ...
    def execute(self, state: PKPopuIndWorkflowState):
    ...
    def execute_directly(
        self,
        state: PKPopuIndWorkflowState,
    ) -> tuple[PKPopuIndCommonAgentResult, Any | None, dict | None]:
        """execute directly"""


class PKPopuIndCommonAgentStep(PKPopuIndCommonStep):
    def __init__(self):
        super().__init__()
    ...
    def get_instruction_prompt(self, state: PKPopuIndWorkflowState):
        """get instruction prompt"""
        return INSTRUCTION_PROMPT

    @abstractmethod
    def get_schema(self) -> PKPopuIndCommonAgentResult | dict:
        """get result schema (pydantic BaseModel or json schema)"""

    @abstractmethod
    def get_post_processor_and_kwargs(
        self, state: PKPopuIndWorkflowState
    ) -> tuple[
        Callable | None,
        dict | None,
    ]:
    ...
    def execute_directly(
        ...
        agent = PKPopuIndCommonAgent(llm=llm)
