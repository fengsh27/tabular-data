...
class PKPopuSumCommonStep(ABC):
    ...
    def enter_step(self, state: PKPopuSumWorkflowState):
        pk_popu_sum_enter_step(state, self.start_title, self.start_descp)

    def leave_step(
        self,
        state: PKPopuSumWorkflowState,
        res: PKPopuSumCommonAgentResult | None = None,
    ...
    def _get_previous_errors_prompt(self, state: PKPopuSumWorkflowState) -> str:
    ...
    def execute(self, state: PKPopuSumWorkflowState):
    ...
    def execute_directly(
        self,
        state: PKPopuSumWorkflowState,
    ) -> tuple[PKPopuSumCommonAgentResult, Any | None, dict | None]:
        """execute directly"""


class PKPopuSumCommonAgentStep(PKPopuSumCommonStep):
    def __init__(self):
        super().__init__()
    ...
    def get_instruction_prompt(self, state: PKPopuSumWorkflowState):
        """get instruction prompt"""
        return INSTRUCTION_PROMPT

    @abstractmethod
    def get_schema(self) -> PKPopuSumCommonAgentResult | dict:
        """get result schema (pydantic BaseModel or json schema)"""

    @abstractmethod
    def get_post_processor_and_kwargs(
        self, state: PKPopuSumWorkflowState
    ) -> tuple[
        Callable | None,
        dict | None,
    ]:
    ...
    def execute_directly(
        ...
        agent = PKPopuSumCommonAgent(llm=llm)
