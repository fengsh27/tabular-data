...
class PatientInfoRefinedResult(PKSpecSumCommonAgentResult):
...
def post_process_refined_patient_info(
    ...
    df_specimen = markdown_to_dataframe(md_table_specimen)
    if not df_table["Population N"].equals(df_specimen["Population N"]):
        error_msg = (
            "Wrong answer example:\n"
            + str(match_list)
            + "\nWhy it's wrong:\nThe rows in the refined Subtable 2 do not correspond to those in Subtable 1 on a one-to-one basis."
        )
        if df_specimen.shape[0] == df_table.shape[0]:
            # check row by row
            list1 = df_table["Population N"].to_list()
            list_patient = df_specimen["Population N"].to_list()
            # check row by row
            for ix in range(len(list1)):
                ...
                if item1 == item2:
                    continue
                logger.error(
                    error_msg + f"\nExpedted df_specimen['Population N']: {list_patient}"
                )
                raise RetryException(error_msg)
        else:
            logger.error(
