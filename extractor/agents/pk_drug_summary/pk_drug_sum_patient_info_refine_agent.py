...
def get_patient_info_refine_prompt(title: str, full_text: str, md_table_drug: str):
...
class PatientInfoRefinedResult(PKDrugSumCommonAgentResult):
...
def post_process_refined_patient_info(
    ...
    if len(match_list) != expected_rows:
        ...
        match_list = [
            pop_n_to_demo.get(
                str(src_row["Population N"]).strip().strip("\"'"), default_demo
            ) + [str(src_row["Population N"]).strip().strip("\"'")]
            for _, src_row in df_drug.iterrows()
        ]
