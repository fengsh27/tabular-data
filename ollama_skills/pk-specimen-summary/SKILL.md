...
cp <final-csv-in-scratch> "$OUT/<pmid>/pk-specimen-summary.csv"
```

If `SKILL_OUTPUT_FOLDER` is unset this writes `./<pmid>/pk-specimen-summary.csv` in the user's current working directory. Then tell the user the exact path you wrote.
