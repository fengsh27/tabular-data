...
1. Take the `Parameter type` column of the sub-table, row by row, keeping each
   row's `Row` join-key value.
...
3. **Process exactly the rows of the sub-table — same count, same `Row`
   values.** For a row you cannot extract, use `N/A` for both values.
...
verbatim from the sub-table) plus the refined type and unit:

```
| Row | Parameter type | Parameter unit |
| --- | --- | --- |
| <row> | <refined type> | <unit> |
```
...
- each output table has the **same `Row` values** as its sub-table (same set,
  same order),
- columns are exactly `Row`, `Parameter type`, and `Parameter unit`.
