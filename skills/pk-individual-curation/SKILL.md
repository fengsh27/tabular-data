...
description: Curate individual-subject pharmacokinetics (PK) tables from a
  biomedical paper into a normalized 12-column dataset (one row per subject per
  parameter). Use when the user provides one or more tables (HTML or markdown)
  reporting per-individual PK data — each row a specific patient/subject/case
  with their own measured values (concentrations, Cmax, AUC, CL, …) — together
  with the table captions and (optionally) the paper title. If several tables
  are supplied, the skill first selects the PK tables among them, then curates
  each. Do NOT use for summary/aggregate PK tables that report mean / median /
  SD / range across a cohort (use pk-summary-curation instead) or for
...
procedure in `skills/curation-common/`.
...
write scratch files under `skills/pk-individual-curation/` (the skill folder is
...
It needs `beautifulsoup4` (see `skills/curation-common/scripts/requirements.txt`).
...
14. **Verification + correction** — `prompts/14_verify_and_correct.md`
    Follows `skills/curation-common/verify_and_correct.md` against the source:
