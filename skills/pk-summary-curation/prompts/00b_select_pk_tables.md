# Stage 0b — Select the PK summary tables

This file is loaded by `SKILL.md` when the input contains **more than one
...
This skill curates **summary statistics**. Among the PK tables, prefer those
that report aggregate statistics (mean / median / SD / range / CI). You do not
need to exclude a table merely because it also contains some per-subject rows —
the individual-data deletion stage (stage 4) handles that downstream.
...
The SKILL.md procedure then runs stages 1–14 once per selected table, each in
