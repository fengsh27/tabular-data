...
For each row, produce these eight fields:

1. **Main value** — the single main value (not a range).
2. **Statistics type** — how the main value was summarized: `Mean`, `Median`,
   `Geometric mean`, `Count`, … **Required — always fill this in.**
...
For each sub-table, a markdown table with exactly the eight columns above, in
order. Use the same `## Sub-table N` headings as `07_subtables.md`.
...
A sub-table row whose value is `6.78 (5.39–8.17)`, summarized as a mean with a
95% CI:

```
| Main value | Statistics type | Variation type | Variation value | Interval type | Lower bound | Upper bound | P value |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 6.78 | Mean | N/A | N/A | 95% CI | 5.39 | 8.17 | N/A |
```
