...
description: Curate pharmacokinetics (PK) summary tables from a biomedical paper
  into a normalized 19-column dataset. Use when the user provides one or more
  tables (HTML or markdown) reporting summary statistics — mean / median / SD /
  range / CI of parameters such as AUC, Cmax, t½, CL, Vd — together with the
  table captions and (optionally) the paper title, and asks for structured
  extraction. If several tables are supplied, the skill first selects the PK
  summary tables among them, then curates each. Do NOT use for individual-subject
  PK tables (use pk-individual-curation instead) or for population/demographic
  PK tables.
---
...
user's data. Never write scratch files under `skills/pk-summary-curation/` (the
...
```
python skills/curation-common/scripts/html_to_markdown_table.py <path-to-html>   # or pipe HTML via stdin
```
...
`beautifulsoup4` (see `skills/curation-common/scripts/requirements.txt`). It is
shared across the curation skills (see `skills/curation-common/`). Only fall
back to converting
...
1. runs `skills/curation-common/scripts/verify_provenance.py` (with
