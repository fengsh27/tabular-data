...
When the drug table has **more than one** row — e.g. a study reporting two
analytes (parent drug + metabolite) or two specimens (maternal plasma + cord
blood) — the shortcut does *not* apply and this matching runs: each data row is
matched to the drug/analyte/specimen combination its parameter column describes.
With a single drug combination, assign it to every row directly (the shortcut)
and skip this prompt.
