...
```
python curation-common/scripts/html_to_markdown_table.py <path-to-html>   # or pipe HTML via stdin
```
...
`beautifulsoup4` (see `curation-common/scripts/requirements.txt`). It is
shared across the curation skills (see `curation-common/`). Only fall
back to converting
...
1. runs `curation-common/scripts/verify_provenance.py` (with
