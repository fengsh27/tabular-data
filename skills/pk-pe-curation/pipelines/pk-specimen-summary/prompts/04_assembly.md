...
- `01_specimen_info.md` — used for its `Specimen` and `Sample N` columns (Stage 1).
...
1. Confirm all three tables have the **same row count**. If not, do not guess —
   note the mismatch, align as many rows as the carried `Population N` allows, and
   report the rest as unresolved.
2. Build each output row in this exact column order:
   `Specimen, Sample N` (from Stage 1) → `Population, Pregnancy stage,
   Pediatric/Gestational age, Population N` (from Stage 2) → `Sample time, Time
   unit` (from Stage 3) → `Note` (the Stage-3 `Source text`, **renamed** to
   `Note`).

That yields the 9 columns:
`Specimen, Sample N, Population, Pregnancy stage, Pediatric/Gestational age,
Population N, Sample time, Time unit, Note`.
...
- exactly 9 columns in the schema order,
