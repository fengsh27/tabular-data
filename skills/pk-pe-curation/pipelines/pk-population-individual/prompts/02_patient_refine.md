...
the parameters below filled in for the PK population individual schema.
...
- **`<KEY-COLUMN>`** = `Patient ID` — carry it through unchanged.
...
same `Patient ID`. Map `Population` to a canonical category (`Nonpregnant` /
