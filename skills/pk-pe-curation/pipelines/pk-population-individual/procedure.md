...
the verify/correct procedure in `curation-common/`.
...
2. **Patient refine** — `prompts/02_patient_refine.md`
   ...
   `curation-common/refine_population.md` with `<KEY-COLUMN>` =
...
5. **Row cleanup** — `prompts/05_row_cleanup.md`
   Runs `curation-common/scripts/clean_population_individual_rows.py` on
