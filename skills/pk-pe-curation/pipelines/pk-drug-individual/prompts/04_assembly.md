...
1. Confirm all three tables have the **same row count**. If they do not, do not
   guess — note the mismatch, align as many rows as the carried `Patient ID` key
   allows, and report the rest as unresolved.
2. Build each output row in this exact column order:
   `Patient ID` (from Stage 2, **first**) → `Drug/Metabolite name, Dose amount,
   Dose unit, Dose frequency, Dose schedule, Dose route` (from Stage 3) →
   `Population, Pregnancy stage, Pediatric/Gestational age` (from Stage 2) →
   `Source text` (from Stage 1, **kept as-is — not renamed**).
3. Drop a row only if it is fully empty / all-`N/A` across every column.

That yields the 11 columns:
`Patient ID, Drug/Metabolite name, Dose amount, Dose unit, Dose frequency, Dose
schedule, Dose route, Population, Pregnancy stage, Pediatric/Gestational age,
Source text`.
...
- exactly 11 columns in the schema order, `Patient ID` first,
- the last column is `Source text` (not `Note`),
