...
def p_pk_summary(
    ...
    if patient_info is None:
        return None
    md_table_patient, res_patient, content_patient, usage_patient, truncated_patient = (
        patient_info
    )
    step_list.append(step_name)
    ...
    if summary_only_info is None:
        return None
    md_table_summary, res_summary, content_summary, usage_summary, truncated_summary = (
        summary_only_info
    )
    step_list.append(step_name)
    ...
    if aligned_info is None:
        return None
    md_table_aligned, res_aligned, content_aligned, usage_aligned, truncated_aligned = (
        aligned_info
    )
    step_list.append(step_name)
    ...
    if mapping_info is None:
        return None
    col_mapping, res_mapping, content_mapping, usage_mapping, truncated_mapping = (
        mapping_info
    )
    step_list.append(step_name)
    ...
    if need_split_col:
        ...
        if split_returns is None:
            return None
        md_table_list, res_split, content_split, usage_split, truncated_split = (
            split_returns
        )
        step_list.append(step_name)
    ...
    for md in md_table_list:
        ...
        if col_name_of_parameter_type in type_unit_cache.keys():
            ...
            if len(col_name_of_parameter_unit_list) == 1:
                ...
                content_to_print = (
                    content_list_clean[-1] if clean_reasoning else content_list[-1]
                )
                print(COLOR_START + "Reasoning:" + COLOR_END)
                print(content_to_print)
    ...
    if need_match_patient is False:
        for md in md_table_list:
            ...
            row_num = df.shape[0]
            df_expanded = pd.concat(
                [markdown_to_dataframe(md_table_patient)] * row_num, ignore_index=True
            )
        ...
        for md in md_table_list:
            ...
            (
                patient_match_list,
                res_patient_match,
                content_patient_match,
                usage_patient_match,
                truncated_patient_match,
            ) = patient_match_info
    ...
    df_combined.loc[
        (df_combined["Lower bound"] == "N/A") & (df_combined["Upper bound"] == "N/A"),
        "Interval type",
    ] = "N/A"
    """if Variation value == "N/A", Variation type must be "N/A"。"""
    df_combined.loc[(df_combined["Variation value"] == "N/A"), "Variation type"] = "N/A"
